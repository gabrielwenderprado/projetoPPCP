...
MODEL_ROW = 1
FIRST_DATA_ROW = 3
OUTPUT_DEFAULT = Path(__file__).resolve().parents[1] / 'data' / 'cilindros.json'
...
def model_name(value, used):
    base = re.sub(r'^cilindro\s*', '', text(value), flags=re.I).strip() or 'Modelo'
...
def extrair(origem: Path, destino: Path) -> None:
    ...
    sheet_name = next((name for name in ('cilindros', 'Cilindros', 'cilindro', 'Cilindro') if name in workbook.sheetnames), None)
    if not sheet_name:
        raise ValueError('A aba cilindros não foi encontrada na planilha.')
    ...
    for block in blocks:
        for row in range(FIRST_DATA_ROW, ws.max_row + 1):
            ...
            item = items.setdefault(code, {
                ...
                'unit': text(ws.cell(row, block['unit']).value) or 'UN',
                'stock': 0,
            ...
            if item['unit'] == 'UN' and text(ws.cell(row, block['unit']).value):
                item['unit'] = text(ws.cell(row, block['unit']).value)
    ...
    print(f'CILINDROS_OK {len(output_items)} itens | {len(model_order)} modelos | {destino}')


if __name__ == '__main__':
    if len(sys.argv) not in {2, 3}:
        raise SystemExit('Uso: python convert_cilindros.py origem.xlsm [destino.json]')
